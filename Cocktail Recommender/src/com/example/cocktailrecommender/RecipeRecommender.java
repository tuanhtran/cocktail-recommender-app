...
import com.example.data.Cocktail;
import com.example.data.RecipeData;
...
public class RecipeRecommender {
	...
	public ArrayList<Cocktail> getRecipesByIngredients(String[] ingredients, boolean strict){
		Cursor cocktailCursor = recipeData.selectByIngredients(ingredients, strict);
		return getCocktailList(cocktailCursor);
	}
	...
	private ArrayList<Cocktail> getCocktailList(Cursor cocktailCursor) {
		...
		if (cocktailCursor.moveToFirst()){
			do {
				...
				try {
					ingredients = getIngredientsFromJSON(cocktailCursor.getString(1));
				...
				Cocktail cocktail = new Cocktail(cocktailName, ingredients, tags, preparation);
	...
	//Expected JSON format: {name:[name1:value1, name2:value2,...]}, where namex is a String representing an Integer (unknown).
	//namex is needed to create SparseArray, so an iterator is created for each JSONArray element (even though the elements have only one key).
	//ToDo: find better way to access namex.
	private SparseArray<String> getIngredientsFromJSON(String ingredientsJSON) throws JSONException {
		SparseArray<String> ingredientsArray = new SparseArray<String>();
		JSONObject ingredients = new JSONObject(ingredientsJSON);
		JSONArray ingredientsJSONArray = ingredients.getJSONArray("ingredients");
